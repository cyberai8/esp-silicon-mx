...
#define LV_CACHE_DEF_SIZE       (524288)
...
#define LV_FONT_FMT_TXT_LARGE 1  /* 和设备一致：中文字体太大，不开这个编不过 */
...
#if LV_USE_FS_POSIX
    #define LV_FS_POSIX_LETTER 'S'      /* 和设备一致：CONFIG_LV_FS_POSIX_LETTER=83 */
...
/* 装了 libsdl2-dev 时 CMake 会定义 SIM_HAS_SDL，于是多出一个 --gui 实时窗口模式；
 * 没装就只走无窗口截图模式。 */
#ifdef SIM_HAS_SDL
    #define LV_USE_SDL          1
#else
